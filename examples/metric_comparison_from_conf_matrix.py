...
for N1 in [100, 500]:
    ...
    out_name = "metric_comparison_with_conf_matrix_P%.2f"%(N1/K)
    ...
    for K10 in np.arange(0,N1,int(N1/40)):
        ...
        for K01 in np.arange(0,N0,int(N0/40)):

            R = utils.compute_R_matrix_from_counts_for_binary_classif(K01, K10, N0, N1)
            
            EC1s.append(avcost.average_cost_from_confusion_matrix(R, priors_unif, costs_01, adjusted=True))
            EC2s.append(avcost.average_cost_from_confusion_matrix(R, priors_data, costs_01, adjusted=True))
            EC3s.append(avcost.average_cost_from_confusion_matrix(R, priors_data, costs_0b, adjusted=True))
